...
TotalFlowRate=KidneyFlowRate+BrainFlowRate+GutFlowRate+LiverFlowRate+MuscleFlowRate
+PancreasFlowRate;
%Remember to remove gut from this later
ArterialSpO2=0.98; % initializing the blood saturation at 98%
VenousMixedSpO2=0.98;%initializing venous SpO2 at same as ArterialSpO2 for testing
the 'nothing happening' condition
...
[Liver,LiverOut]=LiverCalc(LiverFlowRate,Liver,Arterial);%This has to be changed to
include the stream from the gut, will fiddle later
[Muscle,MuscleOut]=MuscleCalc(MuscleFlowRate,Muscle,Arterial);
MixedVenousSpO2=(LiverOut(1)*LiverFlowRate+PancreasOut(1)*PancreasFlowRate+KidneyOu
t(1)*KidneyFlowRate...
+BrainOut(1)*BrainFlowRate+GutOut(1)*GutFlowRate+MuscleFlowRate*MuscleOut(1))/
TotalFlowRate;
MixedVenousGlucose=(LiverOut(2)*LiverFlowRate+PancreasOut(2)*PancreasFlowRate+Kidne
yOut(2)*KidneyFlowRate...
+BrainOut(2)*BrainFlowRate+GutOut(2)*GutFlowRate+MuscleFlowRate*MuscleOut(2))/
TotalFlowRate;
MixedVenousInsulin=(LiverOut(3)*LiverFlowRate+PancreasOut(3)*PancreasFlowRate+Kidne
yOut(3)*KidneyFlowRate...
+BrainOut(3)*BrainFlowRate+GutOut(3)*GutFlowRate+MuscleFlowRate*MuscleOut(3))/
TotalFlowRate;%Will crunch these down into a single matrix operation once I've
